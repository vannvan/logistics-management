<template>
  <div>
    <div class="loginBox">
      ...
      <div class="formBox">
        <div class="inputBox">
            <input type="number" name="" v-model="worker_tel" placeholder="请输入手机号" oninput="if(value.length > 11)value = value.slice(0, 11)">
          <i class="icon-user user-icon" :style="{color:RandomColor()}"></i>
        </div>
        <div class="inputBox">
          <input type="password" name="" v-model="worker_pwd" placeholder="请输入密码">
...
export default {
  ...
  methods: {
    doLogin(){
      ...
      this.$http.post(URL_CONFIG.UrlConfig.workerDoLogin,datas)
      .then(res =>{
        if(res.data.status==1){
          res.data.data.userType="worker"
...
.loginBox{
  ...
  .formBox{
    ...
    margin: auto;
    // background: #fff;
    // margin:pxTorem(400px) auto 0 auto;
  }
  .inputBox{
    position: relative;
  }
