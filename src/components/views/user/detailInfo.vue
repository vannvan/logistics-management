  ...
  <div>
    <group>
      ...
      <cell title="姓名" is-link @click.native="leftPop.show=!leftPop.show;leftPop.type='name'">
        <div>
          <span style="">{{userInfo.student_name}}</span>
        </div>
      </cell>
        ...
        <div class="position-horizontal-demo">
          ...
          <div class="nameOrTelBox">
            ...
            <template v-if="leftPop.type=='name'">
              <input type="text" name="" v-model="student_name">
            </template>
            <template v-if="leftPop.type=='tel'">
              <input type="number" name="" v-model="tel"  oninput="if(value.length > 11)value = value.slice(0, 11)">
            </template>
          </div>
...
export default {
  ...
  mounted(){
    ...
    this.student_name=this.userInfo.student_name
  ...
  methods:{
    ...
    updateInfo(){
      ...
      .then(res =>{
        if(res.data.status==1){
          ...
          this.userInfo.student_name=this.student_name
    ...
    back(){
      if(this.student_name!=this.userInfo.student_name){
