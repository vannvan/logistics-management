<template>
  <div>
    <div class="loginBox">
      ...
      <div class="formBox">
        <div class="inputBox">
          <input type="number" name="" v-model="student_id" placeholder="请输入学号" oninput="if(value.length > 11)value = value.slice(0, 11)">
          <i class="icon-user user-icon" :style="{color:RandomColor()}"></i>
        </div>
        <div class="inputBox">
          <input type="password" name="" v-model="student_pwd" placeholder="请输入密码">
  ...
  methods: {
    doLogin(){
      let datas = {
        ...
        student_pwd:this.student_pwd
      }
      if(this.student_id==''){
        this.$vux.toast.text('学生号不能为空', 'middle')
        return
      }
      if(this.student_pwd==''){
